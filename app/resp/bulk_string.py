...
class BulkString(RESPType[str]):
    ...
    def from_bytes(cls, raw_bytes: bytes) -> tuple[bytes, "BulkString"]:
        ...
        if raw_bytes[0] != BULK_STRING:
            raise WrongRESPFormatError(f"BulkString.from_bytes(): raw_bytes = {raw_bytes!r}")
        ...
        if result_length == -1:
            return raw_bytes[5:], BulkString("")
        start_bulk_string_index = 1 + len(result_length_str) + 2
        bulk_string = read_next_n_bytes(raw_bytes[start_bulk_string_index:], result_length).decode()
